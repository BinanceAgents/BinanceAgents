...
class BlockTimeAnalysisRequest(Model):
    data_url: str = Field(
        description="The url to the blockchain data to be analysed. You MUST use subtask to find blockchain data in CSV."
    )
...
async def handle_message(ctx: Context, sender: str, msg: BlockTimeAnalysisRequest):
    ...
    average_block_time = (
        np.diff(np.flip(blockchain_data["timestamp"])).mean().total_seconds()
    )
...
# Updated: 2025-10-08T20:10:47.693663
